/**
 * POST /api/coding-agents/users/summary
 *
 * F7: histogram consumed by `<SignalSummary>` above the Users
 * directory. Counts DISTINCT users active per time bucket — the
 * sessions histogram (`/api/coding-agents/sessions/summary`)
 * counts chat threads instead, so the two charts never line up
 * when both tabs are open. Mirrors the bucket / label envelope
 * the sessions route returns so `<SignalSummary>` doesn't need
...
// Same USER_EXPR coalesce chain as queries.ts. Kept inline rather
// than imported because the queries module is server-only and
// route handlers may run in an edge runtime.
const USER_EXPR = `
	coalesce(
...
export async function POST(request: Request) {
	...
	try {
		...
		// empty body is OK — same default-window posture as the
	...
	if (user) {
		where.push(`(${USER_EXPR.trim()}) = '${escape(user)}'`);
	}

	// `uniqExact(USER_EXPR)` per time bucket = distinct active users.
	// The "unknown" sentinel is excluded so the histogram doesn't get
	// dominated by pre-user-emit CLIs in mixed-version deployments —
	// it falls under "0 users" for those buckets which is what the
	// directory table also shows.
	const query = `
		SELECT
			formatDateTime(DATE_TRUNC('${bucket}', Timestamp), '${label}') AS label,
			toInt64(uniqExactIf(${USER_EXPR.trim()}, ${USER_EXPR.trim()} NOT IN ('unknown', ''))) AS count
